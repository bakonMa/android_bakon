...
import com.renxin.doctor.activity.nim.event.OnlineStateEventManager;
...
public class NIMInitManager {
    ...
    private void registerLocaleReceiver(boolean register) {
        if (register) {
            ...
            DocCache.getContext().registerReceiver(localeReceiver, filter);
        } else {
            DocCache.getContext().unregisterReceiver(localeReceiver);
        }
    }
    ...
    private void updateLocale() {
        Context context = DocCache.getContext();
    ...
    private void registerBroadcastMessages(boolean register) {
        NIMClient.getService(MsgServiceObserve.class).observeBroadcastMessage(new Observer<BroadcastMessage>() {
            ...
            public void onEvent(BroadcastMessage broadcastMessage) {
                Toast.makeText(DocCache.getContext(), "收到全员广播 ：" +  broadcastMessage.getContent(), Toast.LENGTH_SHORT).show();
            }
        }, register);
    }
